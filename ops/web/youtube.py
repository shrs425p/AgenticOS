...
from ops.web.browser import _ensure_browser, BrowserManager


from kernel.base import tool
class YouTubeMixin:
    @tool(name="youtubeplaynext", desc="Skip to the next YouTube video in the active browser tab.", category="Web")
    @_ensure_browser
    async def youtubeplaynext(self, mgr: BrowserManager) -> str:
    ...
    @tool(name="youtubeplaypause", desc="Toggle play/pause of the YouTube video in the active browser tab.", category="Web")
    @_ensure_browser
    async def youtubeplaypause(self, mgr: BrowserManager) -> str:
    ...
    @tool(name="youtubeskipad", desc="Skip any active advertisement playing on YouTube.", category="Web")
    @_ensure_browser
    async def youtubeskipad(self, mgr: BrowserManager) -> str:
    ...
    @tool(name="findyoutubevideo", desc="Find YouTube video link. Args: query, channel (optional)", category="Web")
    def findyoutubevideo(self, query: str, channel: str = "") -> str:
        ...
        try:
            ...
            timeout = self._get_timeout("websearch", 15)
