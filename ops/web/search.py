...
class SearchMixin:
    @tool(name="websearch", desc="Search the web. Args: query, num_results (optional)", category="Web")
        ...
        try:
            ...
            if not results:
                ...
                if "Search error" in ddg_res or "No results found" in ddg_res:
                    return self._clig_fallback_search(query, n)
                return ddg_res
    ...
    def _clig_fallback_search(self, query: str, n: int = 5) -> str:
        ...
        try:
            ...
            q = urllib.parse.quote_plus(query)
            base_url = self._get_endpoint(
                "clig_search", "https://www.clig.com/search?q={q}"
            )
            url = base_url.format(q=q)
            r = requests_module()
            timeout = self._get_timeout("websearch", 15)
    ...
    @tool(name="searchnews", desc="Search news. Args: query, num_results (optional)", category="Web")
    def searchnews(self, query: str, num_results: str = "5") -> str:
