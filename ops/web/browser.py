"""
AgenticOs — browser automation ops
...
class BrowserMixin:
    ...
    def _require_page(self, mgr: BrowserManager) -> str | None:
        if mgr.page is None:
            return "Error: No browser session active. Call browserlaunch() first."
        return None
    ...
    def _screenshot_path(self, path: str = "") -> str:
        if not path:
            ...
            fmt = self.cfg.get("prompts", {}).get("file_templates", {}).get("browserscreenshot", "browserscreenshot_{ts}.png")
    ...
    @tool(name="browserlaunch", desc="Launch a new Playwright browser or connect to a running Chrome instance via CDP. Args: browser, headless, user_data_dir, cdp_url", category="Web")
    @_ensure_browser
    ...
    @tool(name="browserclose", desc="Close the active browser session and cleanup resources.", category="Web")
    @_ensure_browser
    async def browserclose(self, mgr: BrowserManager) -> str:
        await mgr.cleanup()
        return "Browser session closed."

    @tool(name="browserstatus", desc="Get active browser status (URL, tab count, title).", category="Web")
    @_ensure_browser
    async def browserstatus(self, mgr: BrowserManager) -> str:
        ...
        try:
            ...
            return f"Browser: {mgr.browsertype}\nURL: {url}\nTitle: {title}\nTabs: {n_tabs}"
        except Exception as e:
            return f"Error: {type(e).__name__}: {e}"

    @tool(name="browsernavigate", desc="Navigate the browser to a target URL. Args: url, wait_until", category="Web")
    @_ensure_browser
    ...
    @tool(name="browserscreenshot", desc="Take a screenshot of the active browser page and save to a path. Args: path, full_page", category="Web")
    @_ensure_browser
