...
class WindowsWindowsMixin:
    ...
    @tool(name="windowlist", desc="List windows with titles (Windows). Args: filter_str(optional)", category="Terminal")
    def windowlist(self, filter_str: str = "") -> str:
        """windowlist function."""
        if platform.system() != "Windows":
            return "Error: windowlist is only supported on Windows."
    ...
    @tool(name="windowfocus", desc="Focus a window by title substring (Windows). Args: title", category="Terminal")
    def windowfocus(self, title: str) -> str:
        ...
        """
        if platform.system() != "Windows":
            return "Error: windowfocus is only supported on Windows."
        t = (title or "").strip()
        if not t:
...
"""
    ...
    @tool(name="windowclose", desc="Close a window by title substring (Windows). Args: title", category="Terminal")
    def windowclose(self, title: str) -> str:
        ...
        """
        if platform.system() != "Windows":
            return "Error: windowclose is only supported on Windows."
        t = (title or "").strip()
        if not t:
            return "Error: title required."
        t_escaped = t.replace('"', '`"')
...
"""
    ...
    @tool(name="getbrowserurl", desc="Get the current URL shown in the active browser tab. Args: browser(optional, e.g. 'brave')", category="Terminal")
    def getbrowserurl(self, browser: str = "") -> str:
        ...
        try:
            ...
            self.hotkey("ctrl+c")
            time.sleep(0.35)
            url = self._clipboardget_ps().strip()
    ...
    @tool(name="browserreadpagetext", desc="Read all visible text from the active browser tab (Ctrl+A+C). Args: browser(optional)", category="Terminal")
    def browserreadpagetext(self, browser: str = "") -> str:
        ...
        text = self._clipboardget_ps().strip()
        if not text:
            return "Error: clipboard empty — no text was copied. Is a browser/document window focused?"
        return text

    @tool(name="browserreadselection", desc="Read the currently selected/highlighted text in the browser. Args: browser(optional)", category="Terminal")
    def browserreadselection(self, browser: str = "") -> str:
