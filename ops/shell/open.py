...
from kernel.platformio import PlatformAPI

class OpenersMixin:
    @tool(name="findapp", desc="Find an app executable path (dynamic: PATH/registry/Start Menu). Args: app_name", category="Terminal")
    def findapp(self, app_name: str) -> str:
    ...
    @tool(name="openapp", desc="Open an app (dynamic discovery + launch). Args: app_name, arguments(optional)", category="Terminal")
    def openapp(self, app_name: str, arguments: str = "") -> str:
        """Open an app using dynamic discovery, then launch."""
        found = self.findapp(app_name)
        ...
        if found and found != "Not found.":
            ...
            out = self.launchapplication(found, arguments or "")
            try:
        ...
        return self.launchapplication(app_name, arguments or "")

    @tool(name="launchapplication", desc="Launch desktop application. Args: app_name, arguments (optional)", category="Terminal")
    def launchapplication(self, app_name: str, arguments: str = "") -> str:
        ...
        if self.system == "Windows":
            ...
            if not resolved:
                # Optional cfg-driven candidates if present (set by ToolRegistry).
        ...
        return self.startbackground(cmd)

    @tool(name="openfile", desc="Open file with default application. Args: path", category="Terminal")
    ...
    @tool(name="openurl", desc="Open URL in default browser. Args: url", category="Terminal")
    ...
    @tool(name="openurlverified", desc="Open URL and verify a browser process exists (best-effort). Args: url", category="Terminal")
    def openurlverified(self, url: str) -> str:
        """Open a URL and best-effort verify that a browser process exists."""
        out = self.openurl(url)
        if self.system != "Windows":
            return out
        try:
