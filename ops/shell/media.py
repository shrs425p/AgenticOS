...
class MediaMixin:
    ...
    def _nircmd_path(self) -> str:
        r"""Return path to bundled nircmd.exe (AgenticOs\kernel\nircmd\nircmd.exe)."""
        base = os.environ.get("AGENT_BASE_DIR", "")
        if base:
            p = os.path.join(base, "kernel", "nircmd", "nircmd.exe")
            if os.path.isfile(p):
                return p
        ...
        for _ in range(5):
            ...
            p = os.path.join(here, "kernel", "nircmd", "nircmd.exe")
            if os.path.isfile(p):
                return p
        return ""
    ...
    def _run_nircmd(self, *args: str) -> str:
        ...
        if not nircmd:
            return "Error: nircmd.exe not found in kernel/nircmd/."
        try:
            result = subprocess.run(
                [nircmd, *args],
                capture_output=True,
