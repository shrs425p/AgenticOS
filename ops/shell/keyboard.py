...
class KeyboardMixin:
    ...
    def _resolve_keys(self, keys: str) -> str:
        ...
        Checks self.custom_keys (populated from cfg.yaml custom_keys section).
    ...
    def hotkey(self, keys: str, window: str = "") -> str:
        """Send a keyboard shortcut / hotkey comcliation.

        Args:
            keys  : Key combo OR a custom named shortcut defined in cfg.yaml.
        ...
        Custom shortcuts are defined in cfg.yaml under 'custom_keys'.
        Use hotkeylist to see all defined custom shortcuts.
        """
        raw = (keys or "").strip()
        if not raw:
    ...
    @tool(name="hotkeylist", desc="List all custom named shortcut aliases.", category="Terminal")
    def hotkeylist(self) -> str:
        """List all custom named shortcut aliases defined for this session.

        Shows both shortcuts loaded from cfg.yaml and any added via hotkeyset.
        """
        custom = getattr(self, "custom_keys", {})
        if not custom:
            return "No custom shortcuts defined. Add them in cfg.yaml under 'custom_keys' or use hotkeyset."
        lines = [f"  {name:25s} → {combo}" for name, combo in sorted(custom.items())]
        return f"Custom shortcuts ({len(custom)}):\n" + "\n".join(lines)

    @tool(name="hotkeyset", desc="Define/update a custom named shortcut (session). Args: name, keys", category="Terminal")
    def hotkeyset(self, name: str, keys: str) -> str:
        ...
        Args:
            name: Alias name for the shortcut (e.g. 'screenshot', 'emoji', 'save_all').
                  Use underskernels for multi-word names.
            keys: The key combo to clid (e.g. 'win+shift+s', 'ctrl+shift+s').

        Example:
            hotkeyset(name='screenshot', keys='win+shift+s')
            hotkeyset(name='emoji', keys='win+.')
            hotkeyset(name='lock', keys='win+l')

        Note: Changes are session-only. To persist across restarts, add to
              cfg.yaml under the 'custom_keys' section.
        """
    ...
    @tool(name="hotkeydelete", desc="Remove a custom named shortcut. Args: name", category="Terminal")
    def hotkeydelete(self, name: str) -> str:
    ...
    @tool(name="presskey", desc="Press a single key N times. Args: key (enter/tab/esc/f5/up/etc), repeat(optional)", category="Terminal")
    def presskey(self, key: str, repeat: int = 1) -> str:
    ...
    @tool(name="typetext", desc="Type text as keyboard input. Args: text, delay_ms(optional)", category="Terminal")
    def typetext(self, text: str, delay_ms: int = 0) -> str:
    ...
    @tool(name="keydown", desc="Hold a key down. Args: key (shift/ctrl/alt/etc)", category="Terminal")
    def keydown(self, key: str) -> str:
...
"""
            return self._run_ps(ps_cmd) + f" (keydown: {key})"
    ...
    @tool(name="keyup", desc="Release a held key. Args: key", category="Terminal")
    def keyup(self, key: str) -> str:
...
"""
            return self._run_ps(ps_cmd) + f" (keyup: {key})"
    ...
    @tool(name="mouseclick", desc="Simulate mouse click. Args: button(left/right/middle), x(optional), y(optional)", category="Terminal")
    def mouseclick(self, button: str = "left", x: int = -1, y: int = -1) -> str:
...
"""
    ...
    @tool(name="mousemove", desc="Move mouse cursor to coordinates. Args: x, y", category="Terminal")
...
"""
    ...
    @tool(name="mousescroll", desc="Scroll mouse wheel. Args: direction(up/down), clicks(optional)", category="Terminal")
    def mousescroll(self, direction: str = "down", clicks: int = 3) -> str:
...
"""
    ...
    @tool(name="focuswindowandhotkey", desc="Focus a window then send a hotkey. Args: window, keys", category="Terminal")
    def focuswindowandhotkey(self, window: str, keys: str) -> str:
