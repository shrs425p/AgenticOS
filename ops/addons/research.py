"""Module for researchloop.py"""
import datetime
import json
from pathlib import Path
from kernel.registry import tool
from kernel.settings import DEFAULT_WORKSPACE
from ops.web import WebTools  # noqa: F401

@tool(name="researchloop", desc="Runs a multi-round research loop on a topic.", category="Research")
def researchloop(topic: str, rounds: str = "3") -> str:
    ...
    # Resolve WebTools from this module's attribute so spec that patch
    # "ops.addons.research.WebTools" reliably replace the class.
    web_ops_cls = globals().get("WebTools")
    if web_ops_cls is None:
        from ops.web import WebTools as web_ops_cls
    web_ops = web_ops_cls()
    ...
    for r in range(1, rounds_int + 1):
        ...
        try:
            # First, search
            search_results = web_ops.search(query=current_topic, num_results="3")
            ...
            # Parse out the URLs from the search_results string and use fetchurl on them
            urls_to_fetch = []
            ...
            if urls_to_fetch:
                ...
                for i, url in enumerate(urls_to_fetch[:3]):
                    try:
                        page_text = web_ops.getpagetext(url=url)
