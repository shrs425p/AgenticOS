...
@tool(name="urlsafetycheck", category="Security", desc="Audit a URL's safety, WHOIS registration age, and SSL certificate security profiles.")
def urlsafetycheck(url: str) -> str:
    ...
    and returns a risk skernel (0 to 10) with a detailed assessment report.

    Args:
        url (str): The absolute URL (including protocol, e.g., https://example.com) to analyze.

    Returns:
        str: A detailed markdown report containing the security analysis and threat skernels.
    """
    parsed = urlparse(url)
    ...
    # Bound risk skernel between 0 and 10
    risk_skernel = min(max(risk_skernel, 0), 10)

    # 4. Threat Matrix & Final Risk Summary
    report.append("\n## Threat Matrix Summary\n")
    report.append(f"**Final Risk Skernel**: `{risk_skernel} / 10`")
