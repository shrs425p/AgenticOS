...
def _extract_word_boxes(image_path: str) -> List[Dict[str, Any]]:
    ...
    if not words:
        ...
        from kernel.settings import resolve_local_path
        local_tess = resolve_local_path("ops/Tesseract-OCR/tesseract.exe")
        if os.path.exists(local_tess):
            tesseract_cli = local_tess

        try:
            result = subprocess.run(
                [tesseract_cli, image_path, "stdout", "tsv"],
                capture_output=True,
                text=True,
                encoding='utf-8',
                timeout=30
...
@tool(name="clickelementbyname", desc="Capture screen, find a text element matching the label using OCR coordinate mapping, and click it. Args: label", category="Plugins")
def clickelementbyname(label: str) -> str:
    ...
    try:
        ...
        click_res = term.mouseclick(button="left", x=cx, y=cy)
        return f"Success: Located '{label}' at coordinate ({cx}, {cy}) and triggered click. Click response: {click_res}"
    except Exception as e:
        return f"Error during clickelementbyname: {type(e).__name__}: {e}"

@tool(name="draganddropvisual", desc="Perform visual drag and drop from source text label to destination text label. Args: source, destination", category="Plugins")
def draganddropvisual(source: str, destination: str) -> str:
    ...
    try:
        ...
        if sys_name == "Windows":
            ...
            """
            PlatformAPI.runpowershell(ps_script)
        elif sys_name == "Darwin":
            term = TerminalExecutor()
            term.mousemove(sx, sy)
            time.sleep(0.1)
            term.mouseclick(button="left")
            time.sleep(0.5)
            term.mousemove(dx, dy)
            time.sleep(0.1)
            term.mouseclick(button="left")
        else:
        ...
        return f"Error during draganddropvisual: {type(e).__name__}: {e}"
