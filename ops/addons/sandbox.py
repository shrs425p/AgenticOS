...
def _detect_runtime_path(cliary_name: str) -> str:
    """Finds the absolute path of a cliary executable using shutil.which."""
    path = shutil.which(cliary_name)
...
def _get_active_windows_windows() -> list:
    ...
    try:
        ...
        result = PlatformAPI.runpowershell(cmd, capture_output=True, text=True, timeout=5.0)
        windows = []
...
@tool(name="ossandboxauditor", category="System", desc="Audit the local OS environment, available runtimes, compilers, and active GUI windows.")
def ossandboxauditor() -> str:
    ...
    # Proclig standard runtimes, compilers, and shells
    ...
    # Proclig Active GUI Applications & Desktop Windows
