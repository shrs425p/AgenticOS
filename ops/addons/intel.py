"""Module for competitiveintel.py"""
...
def competitiveintel(competitors: list | None = None) -> str:
    """
    ...
    for competitor in competitors:
        ...
        kernel_feature = "data unavailable"

        try:
            github_search = web_ops.search(f"{competitor} github", num_results="5")
        ...
        try:
            site_search = web_ops.search(f"{competitor} AI agent official site", num_results="2")
            if site_search:
                ...
                if lines:
                    kernel_feature = lines[0][:60] + ("..." if len(lines[0]) > 60 else "")
        except Exception as e:
            logging.warning(f"Failed to fetch site info for {competitor}: {e}")

        matrix_rows.append(f"| {competitor} | {stars} | {last_commit} | {kernel_feature} | Needs Analysis |")
