...
class OCRManager:
    ...
    def __init__(self, rules: dict = None, base_dir: str = "workspace", registry=None, cfg: dict = None):
        ...
        # Get Tesseract path from cfg and resolve it
        media_cfg = self.cfg.get("media", {})
        raw_path = media_cfg.get("tesseract_path", "ops/Tesseract-OCR/tesseract.exe")
    ...
    def ocrimage(self, path: str, engine: str = None) -> str:
        """
        Performs OCR on a local image file.
        Args:
            path: Absolute or workspace-relative path to the image.
            engine: 'tesseract', 'native', or 'auto'. Defaults to cfg.
        """
    ...
    def ocrscreen(self, engine: str = None) -> str:
        """
        ...
        if not self.registry or not hasattr(self.registry, "screen"):
            return "Error: Screen ops not available."
            
        try:
            screenshot_res = self.registry.screen.takescreenshot()
            if "Error" in screenshot_res:
                return screenshot_res
            
            path = screenshot_res.replace("Screenshot saved: ", "").strip()
            return self.ocrimage(path, engine=engine)
        except Exception as e:
