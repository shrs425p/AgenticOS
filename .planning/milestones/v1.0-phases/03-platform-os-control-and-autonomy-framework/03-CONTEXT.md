...
- **New file**: `ops/platform/macos_ui.py` — wraps `osascript` calls and optional pyobjc clidings behind a unified interface.
...
- **New file**: `ops/platform/linux_desktop.py` — detection + screenshot dispatch.
...
- **New file**: `ops/platform/windows_ui.py`.

### Unified Platform API

- **New file**: `ops/platform/__init__.py` — platform dispatcher that detects OS via `sys.platform` / `platform.system()` and routes to the correct backend (`macos_ui`, `linux_desktop`, `windows_ui`).
- `kernel/osio.py` is the existing stub — extend or replace with the new dispatched ops.
...
- **Tiers** (based on total RAM):
  ...
  - Mid (4–16 GB): balanced — default cfg values
  - High (>16 GB): performance — large context, max workers from CPU count
- **New file**: `kernel/resources.py` — profiles hardware and returns a `HardwareProfile` dataclass used to configure the other subsystems at init.
...
- **Integration**: `kernel/tasks.py` extended to write/read checkpoint files. Agent checks for an existing checkpoint at task start (by hashing the goal) and offers to resume.
- **New file**: `kernel/checkpoint.py`.
...
- **New file**: `kernel/triage.py` + `kernel/stalls.py`.

### Deployment Playbook (DOC-01)

- **Format**: Single markdown file `manuals/deployment.md` with sections for:
  - Docker Compose (single-container, volume-mounted workspace)
  - Windows Service via NSSM (`nssm install AgenticOS python main.py`)
  - Kubernetes Deployment manifest (single replica, ConfigMap for cfg.yaml)
...
- `kernel/agent.py` — existing `repeated_action_count` loop guard (retry classifier runs in parallel, does not replace this)
- `kernel/tasks.py` — extended to support checkpoint writes
- `kernel/store.py` — SQLite backend for checkpoint query/lookup
- `kernel/dispatch.py` — `ParallelScheduler.max_workers` is one of the auto-tuned parameters
- `kernel/context.py` — `compact_history` threshold is auto-tuned by `HardwareProfile`
- `kernel/models.py` — `FallbackRouter` handles API transient errors; retry classifier handles tool-execution transient errors (different layer)
- `kernel/osio.py` — existing stub to extend or supersede with platform dispatcher
...
| `repeated_action_count` | `kernel/agent.py` | Loop guard — do not modify, classifier runs alongside |
| `FallbackRouter` | `kernel/models.py` | Handles API retries — retry classifier is for tool execution layer |
| `ParallelScheduler` | `kernel/dispatch.py` | `max_workers` is auto-tuned by `HardwareProfile` |
| `compact_history` | `kernel/context.py` | Compaction threshold auto-tuned by `HardwareProfile` |
| `AgentError` | `kernel/errors.py` | Use for missing-tool and permission errors |
| `@tool` decorator | `kernel/base.py` | Register all new platform ops via this |
