...
- **Exponential Time-Decay**: Incorporated exponential decay weight ($e^{-\lambda \cdot dt}$ where $\lambda = \ln(2)/30$) to decay similarity scores over a 30-day half-life `[ASSUMED]`.
...
- **Episodic Memory Clustering**: Implemented the `cluster_memories` tool allowing users to cluster database namespaces using K-Means and return representative centroid summaries `[ASSUMED]`.
...
- All unit tests in `tests/test_vector_memory.py` run and pass cleanly:
  - `test_vector_memory_store_and_search` (verified basic store/search flow) `[VERIFIED: npm registry]`
  - `test_vector_memory_search_empty` `[VERIFIED: npm registry]`
  ...
  - `test_cluster_memories_tool` (verified correct output structure of representative clusters) `[VERIFIED: npm registry]`
