...
- **PluginRegistryClient Class**: Implemented the downloader client in `kernel/plugins.py` that fetches JSON manifests from a remote registry and downloads raw Python plugin code, saving it locally to `ops/addons/{name}.py` [ASSUMED].
...
- All unit spec in `spec/pluginregistryspec.py` run and pass cleanly [VERIFIED: local pytest]:
...
- Verified that statement and branch code coverage for the newly added `kernel/plugins.py` is at 100% [VERIFIED: local pytest].
