...
Solidify the runtime execution environment against Unicode escape sequences, code generation injection, registry tampering, and symlink traversals. Refactor and modularize `core/runtime.py` into distinct functional layers using Pydantic configuration schemas, a type-safe Tool protocol, and a unified AgentError type.
...
- **D-03:** Load allowed, blocked, and approval-required registry key paths dynamically from `.planning/config.json` schema. Block critical system startup/run hives by default.
...
- **D-05:** Place common interfaces and protocols in `core/tool_base.py`. Register system and custom tools dynamically during runtime initialization to eliminate circular import chains.
...
- Precise directory structure boundaries for modular sub-modules under `core/`.
...
- `docs/security_guardrails.md` — Current pathguard design guidelines.
- `docs/command_validation.md` — Existing command sanitization and block rules.
...
### Reusable Assets
- `PathGuard` in `core/guardrails.py`: Checks file path boundaries and zone restrictions.
- `SafetyMixin` in `tools/terminal/safety.py`: Handles regex tokenization and command verb checks.
...
### Integration Points
- Command validation hooks directly into terminal runners in `tools/terminal/runner.py`.
- Modularized sub-modules will be loaded and initialized in `core/runtime.py`.
