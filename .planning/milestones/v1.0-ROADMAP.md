...
**Goal**: Build AST unicode guards, Pydantic action models, modularize runtime, and establish security regression spec.
**Depends on**: Nothing
...
**Success Criteria** (what must be TRUE):
  ...
  4. Large functions in `kernel/cli.py` are cleanly separated into orchestrator, dispatcher, memory, and error files.
...
**Success Criteria** (what must be TRUE):
  ...
  3. Independent ops in execution dependency graphs run concurrently.
...
**Success Criteria** (what must be TRUE):
  ...
  2. The system executes async ops and pipes output streams sequentially.
  3. Remote plugin registry downloads and installs compatible custom ops.
  4. Chaos monkey harness spec framework resiliency under API failure and database corruption scenarios.
