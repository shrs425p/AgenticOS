...
## What was done
- Developed a system resource profiler (`core/resource_profiler.py`) utilizing `psutil` to analyze host RAM limits, CPU counts, and system load.
...
- Created a robust checkpoints manager (`core/checkpoint_manager.py`) mapping execution tasks to stable 12-char SHA256 hashes of goals.
...
- Wrote unit tests in `tests/test_resource_profiler.py` mocking various CPU/RAM values to verify correct tier categorization and config suggestions.
- Wrote unit tests in `tests/test_checkpoint_manager.py` using temp files and databases to verify serialization, loading, and next-phase resumption transitions.
