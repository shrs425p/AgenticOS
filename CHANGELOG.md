...
- **Command Validation Reference**: Added `manuals/commands.md` documenting the zero-trust validator, script parser, and PowerShell audits.
- **Self-Improvement Guide**: Added `manuals/improve.md` detailing the dreaming reflections engine, logs audits, and offline heuristics.
- **Developer Quick Start**: Added `manuals/quick.md` containing a fast 2-minute setup, CLI run command, and standard agent workflows.
- **Core and Package Docs Update**: Synchronized `manuals/guard.md`, `manuals/architecture.md`, `manuals/testing.md`, `manuals/CATALOG.md`, `kernel/README.md`, `ops/README.md`, and `spec/README.md`.
...
- **Optimized Hot-Reload Checks**: Refactored `_get_mtimes` inside `kernel/cli.py` to exclude dependency/data directories (`venv`, `node_modules`, `workspace`, `data`, and `mock_workspace`) from the file scanning traversal. This reduces idle CPU usage and disk I/O significantly.
- **Optimized Workspace Context Scan**: Updated `_scan_workspace` inside `kernel/context.py` to skip scanning and child-counting for heavy or system-generated directories (`.git`, `venv`, `node_modules`, `__pycache__`, caches, and data directories) during prompt assembly.

### Documentation & Verification
- **Unified Tool Count & Metrics**: Updated `manuals/index.html` and other documentation files to reflect the actual registry size of **352** specialized ops (previously reported as 180+) and accurate performance benchmarks.
- **Visual Diagram Corrections**: Replaced the outdated PowerShell pipeline diagram and description in `manuals/visual.md` with the new optimized DFS stack walker flowchart, and resolved a code fence formatting syntax error.
...
- **Concurrency Controls**: Added concurrency groups across all active GitHub Workflows (`build.yml`, `audit.yml`, `codeql.yml`, `deps.yml`) to automatically cancel redundant builds on subsequent pushes.
- **Fixed Hallucinated Action Versions**:
  - Corrected broken `actions/upload-artifact@v7` (non-existent) references to `@v4` in `build.yml` and `deps.yml`.
  - Corrected broken `actions/checkout@v6` (non-existent) references to `@v4` across `audit.yml`, `codeql.yml`, `deps.yml`, and `summary.yml`.
- **Protected Security Auditing Rules**: Safeguarded essential Bandit scan command skips (`B101,B603,B602,B605,B607,B404`) and folder exclusions (`ops/terminal`) to prevent false-positive command execution alerts on agent terminal ops.
...
- **CodeQL Alert Mitigation**: Resolved CodeQL Medium CWE-829 (unpinned-tag) alert by pinning `codecov/codecov-action` inside `build.yml` to its immutable full-length commit SHA (`57e3a136b779b570ffcdbf80b3bdc90e7fab3de2`).
...
- **Upgraded PR Template**: Re-engineered [.github/PULL_REQUEST_TEMPLATE.md](.github/PULL_REQUEST_TEMPLATE.md) to integrate customized checks for AgenticOS tool registry, shadow testing (`alltoolsshadowspec.py`), command execution safety, and dynamic path portability checks.
...
- Resolve false-positive CodeQL CWE-20 Incomplete URL substring sanitization warnings in unit test assertions (`websearchspec.py`, `webtoolsspec.py`, `webfetchspec.py`).
- Automatically verify and create the tasks directory inside `workspace` if missing in `kernel/tasks.py` to prevent initialization errors.
- Silently suppress duplicate tool registration console logs in `kernel/registry.py` to keep boot output clean.
- Fix async mock behaviors in `spec/webbrowserspec.py` to properly handle `AsyncMock` context engines.
...
- Fully align all developer guides and user manuals (`manuals/interface.md`, `manuals/tools.md`) to run the standard `agent` launcher globally instead of the legacy `python main.py` command.

### Maintenance and Refactors
- Add `kernel/retry.py` and migrate provider clients to use a centralized `retry_call()` helper for exponential backoff and jittered retries.
...
- Namespaced plugin imports under `ops.addons.<module>` and updated `ToolRegistry` to register top-level `@tool` callables, improving hot-reload and test patchability.
- Add `kernel/schema.py` types and annotate `kernel/settings.py` for clearer cfg typing.
- Add `spec/retryspec.py` to cover retry behaviour.

## [2.0.1] - 2026-05-16

### New Features
- Daily maintenance plugins framework: `feat: add daily maintenance plugins and spec (#15)` (Shreyas Pawar)
