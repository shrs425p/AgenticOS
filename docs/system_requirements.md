...
-   **OS**: Windows 10/11 (Version 21H2 or later).
...
### 2. Native Tools (Windows)
AgenticOS relies on several native Windows utilities for its "Hardened" performance:
-   **PowerShell 5.1+**: Standard on Windows 10/11.
-   **WMI (Windows Management Instrumentation)**: Enabled by default for system health monitoring.
-   **Playwright Dependencies**: Requires the Chromium/Firefox browser binaries (installed via `playwright install`).
...
The agent will call `system_health` and compare your RAM, CPU, and VRAM against the targets listed in this document.
