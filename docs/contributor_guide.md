...
- Follow the [Setup Guide](file:///c:/Users/pawar/AgenticOS/docs/setup_guide.md) to set up Python and install required scripts.
- Refer to the [Developer Onboarding Guide](file:///c:/Users/pawar/AgenticOS/docs/developer_onboarding.md) for local dev dependencies and code verification commands.
...
- **Tool Development**: Decorate your system or custom tools using `@tool` from `core/tool_base.py` and register them in the registry.
...
2. Implement your changes. Ensure you write robust unit tests under `tests/` matching your new feature's domain.
3. Verify that all 612 tests in the test suite pass successfully:
   ```bash
   venv\Scripts\pytest
   ```
