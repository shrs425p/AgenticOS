"""Unit spec for the Multi-Modal Visual Coordinate Mapping Engine."""
from unittest.mock import MagicMock, patch

from ops.addons.vision import _extract_word_boxes, _find_phrase_coords, clickelementbyname, draganddropvisual
...
def test_clickelementbyname_success():
    with patch("ops.addons.vision._extract_word_boxes") as mock_extract, \
         patch("ops.screen.ScreenManager.takescreenshot", return_value="Screenshot saved: dummy.png"), \
         patch("os.path.exists", return_value=True), \
         patch("ops.shell.TerminalExecutor.mouseclick", return_value="Clicked left at (30, 30).") as mock_click:
        mock_extract.return_value = [
            {"text": "File", "x": 10, "y": 20, "w": 40, "h": 20}
        ]

        res = clickelementbyname("File")
        assert "Success" in res
        assert "(30, 30)" in res
        mock_click.assert_called_once_with(button="left", x=30, y=30)

def test_draganddropvisual_win():
    with patch("ops.addons.vision._extract_word_boxes") as mock_extract, \
         patch("ops.screen.ScreenManager.takescreenshot", return_value="Screenshot saved: dummy.png"), \
         patch("os.path.exists", return_value=True), \
        ...
        res = draganddropvisual("Source", "Dest")
        assert "Success" in res
