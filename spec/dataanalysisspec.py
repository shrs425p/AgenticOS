"""Unit spec for the data analysis plugin."""
...
def test_datasummarizecsv(temp_csv):
    result = datasummarizecsv(temp_csv)
...
def test_data_summarize_not_found():
    result = datasummarizecsv("nonexistent_file.csv")
...
def test_datageneratechart_line(temp_csv, temp_output):
    result = datageneratechart(
        temp_csv, 
        x_column="Date", 
...
@pytest.mark.skipif(not HAS_PANDAS or not HAS_MATPLOTLIB, reason="pandas/matplotlib not installed")
def test_datageneratechart_bar(temp_csv, temp_output):
    result = datageneratechart(
        temp_csv, 
        x_column="Date", 
        y_columns=["Sales"], 
...
@pytest.mark.skipif(not HAS_PANDAS or not HAS_MATPLOTLIB, reason="pandas/matplotlib not installed")
def test_datageneratechart_invalid_column(temp_csv, temp_output):
    result = datageneratechart(
        temp_csv, 
        x_column="InvalidCol", 
        y_columns=["Sales"], 
...
@pytest.mark.skipif(not HAS_PANDAS or not HAS_MATPLOTLIB, reason="pandas/matplotlib not installed")
def test_datageneratechart_invalid_type(temp_csv, temp_output):
    result = datageneratechart(
        temp_csv, 
        x_column="Date", 
        y_columns=["Sales"], 
