...
from ops.addons.healtest import selfhealingtest

def test_selfhealingtest_happy_path(tmp_path, monkeypatch):
    # Mock workspace and daily logs
    monkeypatch.chdir(tmp_path)
    workspace = tmp_path / "workspace"
    workspace.mkdir()

    import ops.addons.healtest
    monkeypatch.setattr(ops.addons.healtest, "DEFAULT_WORKSPACE", str(workspace))

    # Run the test
    res_str = selfhealingtest()
...
def test_selfhealingtest_workspace_readonly(tmp_path, monkeypatch):
    # Mock workspace and daily logs
    monkeypatch.chdir(tmp_path)
    workspace = tmp_path / "workspace"
    workspace.mkdir()
...
def test_selfhealingtest_log_file_write_fail(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    workspace = tmp_path / "workspace"
    workspace.mkdir()
