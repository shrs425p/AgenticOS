...
# 1. Add <REPO_ROOT> to sys.path first (spec/ is 1 level below root)
...
def run_shadow_spec():
    """Run all registered ops in shadow mode, build and write the completed report."""
    ...
    def mock_get(*a, **k):
        return MagicMock(status_code=200, text="Simulated HTML/API response", content=b"Simulated cliary", json=lambda: {"status": "success"})
    ...
    with stack:
        ...
        try:
            ...
            print(f"Loaded registry. Total ops: {len(registry.registry)}")
...
"""
            ...
            report_path = os.path.join(workspace_reports_dir, "all_ops_status_report_completed.md")
            
            with open(report_path, "w", encoding="utf-8") as f:
                f.write(report_content)

            print(f"Successfully generated completed tool status report with {len(registry.registry)} ops!")
