from kernel.validate import _normalize_path, _resolve_path, validate_tool, _arg_get
...
def test_validate_tool(tmp_path):
    ...
    # downloadfile - exists
    res = validate_tool("downloadfile", {"dest_path": str(test_file)}, "", workspace_root=tmp_path)
    assert res == "VALIDATION: download exists"

    # terminal
    res = validate_tool("runcommand", {}, "[exit: 0]", workspace_root=tmp_path)
    assert res == "VALIDATION: command returned exit status (see output)"

    # openurl
    assert validate_tool("openurl", {}, "", workspace_root=tmp_path) == ""
