...
def test_profile_hardware_low():
    ...
    with patch("kernel.resources.psutil.virtual_memory", return_value=mock_mem), \
         patch("kernel.resources.os.cpu_count", return_value=2):
...
def test_profile_hardware_mid():
    ...
    with patch("kernel.resources.psutil.virtual_memory", return_value=mock_mem), \
         patch("kernel.resources.os.cpu_count", return_value=4):
...
def test_profile_hardware_high():
    ...
    with patch("kernel.resources.psutil.virtual_memory", return_value=mock_mem), \
         patch("kernel.resources.os.cpu_count", return_value=8):
