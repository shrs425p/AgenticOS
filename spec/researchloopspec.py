...
from ops.addons.research import researchloop

@patch("ops.addons.research.WebTools")
def test_researchloop_happy_path(mock_web_ops, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    workspace = tmp_path / "workspace"
    workspace.mkdir()

    import ops.addons.research
    monkeypatch.setattr(ops.addons.research, "DEFAULT_WORKSPACE", str(workspace))

    # Setup mock WebTools
    mock_instance = MagicMock()
    mock_web_ops.return_value = mock_instance
    ...
    # Run tool
    res_str = researchloop(topic="AI Agents", rounds="3")
...
@patch("ops.addons.research.WebTools")
def test_researchloop_round_2_fails(mock_web_ops, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    workspace = tmp_path / "workspace"
    workspace.mkdir()
...
@patch("ops.addons.research.WebTools")
def test_researchloop_all_rounds_fail(mock_web_ops, tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    workspace = tmp_path / "workspace"
    workspace.mkdir()
...
def test_researchloop_empty_topic(tmp_path, monkeypatch):
    res_str = researchloop(topic="", rounds="3")
    result = json.loads(res_str)
    assert result["status"] == "handled gracefully"
    assert result["reason"] == "Empty topic"

def test_researchloop_zero_rounds(tmp_path, monkeypatch):
    res_str = researchloop(topic="AI Agents", rounds="0")
    result = json.loads(res_str)
    assert result["status"] == "handled gracefully"
