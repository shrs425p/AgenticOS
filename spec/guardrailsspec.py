from kernel.guard import PathGuard
...
def test_guardrails_workspace_green_zone_exception(tmp_path):
    ...
    from unittest.mock import patch
    with patch("kernel.guard.Path") as mock_path:
...
def test_blue_zone_disabled_by_default(tmp_path):
    """read_only defaults to False when not set in cfg."""
    ws = tmp_path / "workspace"
    ws.mkdir()
    cfg = {"agent": {"workspace": str(ws)}}
    guard = PathGuard(cfg)
...
def test_guardrails_sensitive_shields(tmp_path):
    """PathGuard strictly shields sensitive credential/cfg files and tamper-proofs audit logs."""
    ws = tmp_path / "workspace"
    ws.mkdir()
    cfg = {"agent": {"workspace": str(ws)}}
    guard = PathGuard(cfg)
    ...
    allowed, msg = guard.check_path(str(ws / ".git" / "cfg"), "read")
    assert not allowed
    assert "strictly blocked" in msg

    # 3. Config files (cfg.yaml, cfg/*) read allowed, but modify/delete strictly blocked
    allowed, _ = guard.check_path(str(ws / "cfg.yaml"), "read")
    assert allowed

    allowed, msg = guard.check_path(str(ws / "cfg.yaml"), "write")
    assert not allowed
    assert "strictly blocked" in msg

    allowed, _ = guard.check_path(str(ws / "cfg" / "policy.yaml"), "read")
    assert allowed

    allowed, msg = guard.check_path(str(ws / "cfg" / "policy.yaml"), "delete")
    assert not allowed
    assert "strictly blocked" in msg
