...
from kernel.improve import SelfImprovementDaemon, run_dream_cycle
...
def test_dream_cycle_with_llm(workspace):
    ...
    tracking_file.write_text(json.dumps({
        "completed_tasks": [
            ...
            {"goal": "make coffee", "success": True, "duration": 300, "ops_used": ["cup"]},
        ]
    }))
...
def test_dream_cycle_fallback(workspace):
    ...
    tracking_file.write_text(json.dumps({
        "completed_tasks": [
            {"goal": "make bug", "success": False, "duration": 10},
            {"goal": "make coffee", "success": True, "duration": 300, "ops_used": ["cup", "cup", "spoon"]},
        ]
    }))
...
def test_offline_fallback_scan(workspace, monkeypatch):
    ...
    monkeypatch.setattr("kernel.improve.Path", MockPath)
