"""Unit spec for SafetyMixin structural command validation and obfuscation detection."""
...
# ---------------------------------------------------------------------------
# Wave 2 — Phase 3: Runner Integration spec
# ---------------------------------------------------------------------------


class DummyRunner(SafetyMixin, RunnerMixin):
    """Minimal stub comcliing SafetyMixin + RunnerMixin for testing.
...
def test_script_not_found():
    ...
    runner = DummyRunner(_default_rules())
    result = runner.runscript("/nonexistent/path/foo.sh")
...
def test_powershell_flag_abbreviations_and_case():
    ...
    assert not safety._is_powershell_command_flag("-cfg")
