...
def test_vector_memory_decay_mutation():
    """Verify that mutating the decay factor formula in vector_memory.py causes spec to fail."""
    ...
    try:
        ...
        # Run pytest on spec/vectormemoryspec.py
        cmd = [sys.executable, "-m", "pytest", "spec/vectormemoryspec.py", "-k", "test_exponential_time_decay"]
...
def test_vector_memory_sort_direction_mutation():
    """Verify that negating the similarity search sorting direction causes spec to fail."""
    # 1. Read original content
    with open(VECTOR_MEMORY_PATH, "r", encoding="utf-8") as f:
        original_content = f.read()
    ...
    try:
        ...
        # Run pytest on spec/vectormemoryspec.py
        cmd = [sys.executable, "-m", "pytest", "spec/vectormemoryspec.py", "-k", "test_vectormemorystore_and_search"]
        result = subprocess.run(cmd, capture_output=True, text=True)
