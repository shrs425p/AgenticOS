...
@patch("ops.web.fetch.requests_module")
def test_fetchurl(mock_req):
    tool = DummyWebTool()
    ...
    res = tool.fetchurl("https://example.com")
...
@patch("ops.web.fetch.bs4_beautifulsoup")
def test_getpagetext(mock_bs4, mock_get):
    tool = DummyWebTool()
    mock_resp = MagicMock()
    ...
    res = tool.getpagetext("https://example.com")
    assert res == "Hello"
...
@patch("requests.Session.get")
@patch("ops.web.fetch.bs4_beautifulsoup")
def test_getpagelinks(mock_bs4, mock_get):
    tool = DummyWebTool()
    mock_resp = MagicMock()
    mock_resp.status_code = 200
    ...
    res = tool.getpagelinks("https://example.com")
...
@patch("requests.Session.get")
@patch("ops.web.fetch.bs4_beautifulsoup")
def test_getpageimages(mock_bs4, mock_get):
    tool = DummyWebTool()
    mock_resp = MagicMock()
    mock_resp.status_code = 200
    ...
    from bs4 import BeautifulSoup
    mock_bs4.return_value = BeautifulSoup
    res = tool.getpageimages("https://example.com")
    assert ("https://example" + ".com/img.png") in res

@patch("requests.Session.get")
def test_downloadfile(mock_get, tmp_path):
    tool = DummyWebTool(workspace_dir=str(tmp_path))
    mock_resp = MagicMock()
    mock_resp.iter_content.return_value = [b"chunk1", b"chunk2"]
    mock_get.return_value = mock_resp
    res = tool.downloadfile("https://example.com/file", "out.txt")
