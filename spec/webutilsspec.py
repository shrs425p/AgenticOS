...
def test_shortenurl(mock_req):
    mock_resp = mock.MagicMock()
    mock_resp.text = " https://is.gd/short "
    mock_req().get.return_value = mock_resp
    
    tool = MockWebTools()
    assert tool.shortenurl("http://google.com") == "https://is.gd/short"
    
    # Network error
    tool._network_err = "No Network"
    assert tool.shortenurl("http://google.com") == "No Network"

@mock.patch("ops.web.utils.requests_module")
def test_expandurl(mock_req):
    mock_resp = mock.MagicMock()
    mock_resp.url = "https://google.com"
    mock_req().head.return_value = mock_resp
    
    tool = MockWebTools()
    assert tool.expandurl("https://is.gd/short") == "https://google.com"
    
    # Exception handling
    mock_req().head.side_effect = Exception("Expand failed")
    assert "Expand error" in tool.expandurl("https://is.gd/short")

@mock.patch("ops.web.utils.bs4_beautifulsoup")
@mock.patch("ops.web.utils.requests_module")
def test_rssfeed(mock_req, mock_bs4):
    mock_resp = mock.MagicMock()
    ...
    tool = MockWebTools()
    assert "A | B | C" in tool.rssfeed("https://feed.xml")

@mock.patch("ops.web.utils.requests_module")
def test_waybacksnapshot(mock_req):
    mock_resp = mock.MagicMock()
    mock_resp.text = '{"snapshot": "ok"}'
    mock_req().get.return_value = mock_resp
    
    tool = MockWebTools()
    assert tool.waybacksnapshot("http://google.com") == '{"snapshot": "ok"}'

@mock.patch("ops.web.utils.bs4_beautifulsoup")
def test_scrapetable(mock_bs4):
    tool = MockWebTools()
    ...
    with mock.patch.object(tool, "_get_session", return_value=session):
        ...
        res = tool.scrapetable("http://table.com")
        assert "Cell content" in res
        
        # Table out of range
        assert "out of range" in tool.scrapetable("http://table.com", "5")
        
        # No tables found
        soup.find_all.return_value = []
        assert "No tables found" in tool.scrapetable("http://table.com")
