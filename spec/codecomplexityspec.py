"""Tests for the codecomplexity plugin."""
import os
from unittest.mock import MagicMock, patch
from ops.addons.complexity import (
...
def test_codecomplexity_invalid_file():
    """Verify error returns for missing or non-Python files."""
    # Test missing file
    result = codecomplexity("missing_file.py")
    ...
    with open("temp_test.txt", "w") as f:
        f.write("test")
    try:
        result = codecomplexity("temp_test.txt")
...
@patch("ops.addons.complexity._ensure_radon_installed")
@patch("radon.visitors.ComplexityVisitor.from_code")
def test_codecomplexity_analysis(mock_visitor_from_code, mock_installed):
