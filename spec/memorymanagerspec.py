...
def test_generate_insights_with_llm(workspace):
    ...
    tasks = [
        {"goal": "G1", "success": True, "duration": 5, "ops_used": ["T1"], "timestamp": "2024-01-01T00:00:00"},
        {"goal": "G2", "success": False, "duration": 10, "ops_used": ["T1", "T2"], "timestamp": "2024-01-01T00:01:00"}
    ]
...
def test_global_helpers(workspace):
    ...
    kernel.memory._memory_manager = None
...
def test_consolidate_long_term_memory_llm_json_fallback(workspace):
    ...
    mm = MemoryManager(str(workspace), llm_client=client)
    tasks = [{"goal": "G1", "success": True, "duration": 5, "ops_used": ["T1"], "timestamp": "2024-01-01T00:00:00"}]
    insights = mm._generate_insights_from_tasks(tasks)
...
def test_cleanup_old_memories_exception(tmp_path):
    from kernel.memory import MemoryManager
    # This spec the 'except Exception:' inside the glob loop
    ws = tmp_path / "workspace"
    ws.mkdir(exist_ok=True)
    mm = MemoryManager(str(ws))
...
def test_is_meaningful_task():
    from kernel.memory import is_meaningful_task
