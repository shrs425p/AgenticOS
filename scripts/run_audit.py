...
with open("tests/auto/test_autogenerated.py", "w", encoding="utf-8") as f:
    ...
    f.write("def mock_external_calls(monkeypatch):\n")
    f.write("    monkeypatch.setattr(os, 'system', MagicMock(return_value=0))\n")
    f.write("    monkeypatch.setattr(subprocess, 'run', MagicMock())\n")
    f.write("    monkeypatch.setattr(subprocess, 'Popen', MagicMock())\n")
    f.write("    monkeypatch.setattr(requests, 'get', MagicMock())\n")
    f.write("    monkeypatch.setattr(requests, 'post', MagicMock())\n")
    f.write("    monkeypatch.setattr(webbrowser, 'open', MagicMock())\n")
    f.write("    monkeypatch.setattr(shutil, 'which', MagicMock(return_value='/usr/bin/mock'))\n\n")
