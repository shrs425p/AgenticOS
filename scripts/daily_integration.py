...
def run_smoke_tests(registry):
    ...
    for category, tools in categories.items():
        ...
        tool_name = tools[0]
        for t in tools:
            if t in ["read_file", "run_command", "web_search", "system_info", "browser_read_page_text", "plugin_health_check"]:
                tool_name = t
                break
...
def run_tool_chains(registry):
    ...
    process_tool = "word_count" if "word_count" in reg else ("grep_file" if "grep_file" in reg else None)
    ...
    if "run_command" in reg:
        try:
            ...
            chain_results.append(("terminal_command -> capture_output -> parse_result", f"FAIL ({str(e)})"))
...
def compare_with_yesterday(results):
    ...
    for r in results:
        if r['tool'] in yesterday_passing and r['pass_fail'] == 'FAIL':
            flags.append(f"FLAG: Tool {r['tool']} was passing yesterday but is failing today.")

    return flags

def generate_markdown_report(results, failed_tools, error_recovery_pass, chain_results, flags):
    ...
    report_lines.append("|------|-------|-------------|-----------|-----------|")
    for r in results:
        report_lines.append(f"| {r['tool']} | `{r['input']}` | {r['output_type']} | {r['pass_fail']} | {r['time_ms']} |")
    report_lines.append("")
    ...
    report_lines.append(f"- Missing/Invalid argument handling: **{'PASS' if error_recovery_pass else 'FAIL'}**")
    report_lines.append("")
    ...
    health = "GOOD"
    if failed_tools or not error_recovery_pass or any("FAIL" in status for _, status in chain_results):
        health = "WARNING"
...
def main():
    ...
    report_content = generate_markdown_report(results, failed_tools, error_recovery_pass, chain_results, flags)
