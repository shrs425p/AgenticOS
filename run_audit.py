...
with open("tests/auto/test_autogenerated.py", "w") as f:
    ...
    for name, info in missing_tests:
        ...
        f.write(f"    assert tool_info is not None\n")
        f.write(f"    func = tool_info['fn']\n")
        if info["fn"].__name__ == "<lambda>" and ("open_" in name or "search_" in name):
             f.write(f"    # Lambda wrapper or preset tool\n")
             f.write(f"    if 'value' in inspect.signature(func).parameters:\n")
             f.write(f"        func(value='mock')\n")
             f.write(f"    else:\n")
             f.write(f"        func()\n")
        else:
...
with open(f"workspace/daily_logs/tool_audit_{date_str}.md", "w") as f:
