"""Startup cfg validator for AgenticOs.

Validates the fully-merged cfg dict (root cfg.yaml + cfg/*.yaml layers)
...
* Be layered-cfg-aware: only flag keys that are absent *after* all layers
...
# Keys recognised at the *root* of cfg.yaml that override layered values.
...
_KNOWN_ROOT_KEYS = {
    ...
    "prompts", "policy", "ops", "terminal", "browser", "media",
...
def _check_agent_section(cfg: dict, result: ValidationResult) -> None:
    ...
    if not provider:
        result.add(
            ...
            "Add 'agent:\\n  provider: ollama' (or nvidia/gemini/…) to cfg.yaml.",
        )
    ...
    if provider in _PROVIDER_ENV_KEYS:
        ...
        if not model:
            result.add(
                ...
                f"in cfg/{provider}.yaml or cfg/providers.yaml.",
            )
...
def _check_unknown_root_keys(root_cfg: dict, result: ValidationResult) -> None:
    """Warn about keys in the *root* cfg.yaml that aren't recognised.

    This helps catch typos like ``agnet:`` or ``automony:``.
    We receive the root file's raw dict separately from the merged cfg
    ...
    for key in sorted(unknown):
        result.add(
            "WARNING",
            f"Unrecognised top-level key '{key}' in cfg.yaml.",
...
def _check_ollama_section(cfg: dict, result: ValidationResult) -> None:
    ...
    if not base_url:
        result.add(
            ...
            "to cfg/providers.yaml (or cfg.yaml).",
        )
...
def validate_cfg(
    ...
    *,
    ...
    """Run all checks against the fully-merged cfg.

    Parameters
    ----------
    merged_cfg:
        The dict returned by ``load_cfg()`` after all layer merges.
    root_cfg:
        (Optional) The raw dict loaded *only* from the root ``cfg.yaml``
...
def warn_cfg_issues(
    ...
    *,
    ...
    """
    result = validate_cfg(merged_cfg, root_cfg=root_cfg)
    if not result.issues:
        return result

    try:
        from kernel.ui import print_warning, print_error, print_info  # noqa: PLC0415
        _warn = print_warning
        _err  = print_error
        _info = print_info
    except Exception:
