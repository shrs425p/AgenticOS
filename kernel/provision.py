...
PLUGINS_DIR = os.path.join(BASE_DIR, "ops", "addons")
...
def refresh_path() -> None:
    ...
    if platform.system() == "Windows":
        ...
        choco_links = os.path.join(os.environ.get("ProgramData", os.environ.get("SystemDrive", "C:") + "\\ProgramData"), "chocolatey", "cli")
...
def self_provision_command(command_name: str) -> bool:
    ...
    Args:
        command_name (str): Name of the command cliary to verify/install.
    ...
    if not cmd_clean:
        return False
    toolname = "".join(ch for ch in cmd_clean if ch.isalnum())
    if not toolname:
        return False
    ...
    try:
        from ops.addons.package import installsystempackage
        res = installsystempackage(cmd_clean)
    except Exception as e:
        logging.error(f"Self-provisioner: Failed to import/execute installsystempackage: {e}")
        return False
    ...
    plugin_file = os.path.join(PLUGINS_DIR, f"auto{toolname}.py")
...
@tool(name="auto{toolname}", desc="Auto-generated system tool to run the {cmd_clean} command. Args: args (a string of command arguments)", category="Plugins")
def auto{toolname}(args: str) -> str:
