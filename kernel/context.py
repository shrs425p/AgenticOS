...
from kernel.settings import DEFAULT_SCAN_EXCLUDED_DIRS
...
class ContextEngine:
    ...
    def build_system_prompt(self, recall_context: str = "", commitments_context: str = "") -> str:
        ...
        # Get unified system prompt from cfg
        raw_prompt = self.cfg.get("prompts", {}).get(
            "system_prompt", "You are an AI assistant."
        )
        ops_block = self.agent.ops.tool_descriptions()
        ...
        if "{tool_descriptions}" in raw_prompt:
            system = raw_prompt.replace("{tool_descriptions}", ops_block)
        else:
            system = f"{raw_prompt}\n\nAVAILABLE_TOOLS:\n{ops_block}"
        ...
        # 4. Thinking Canvas
        canvas_block = ""
        if self.agent.ops._canvas:
            canvas_tmpl = ctx_prompts.get("thinking_canvas", "### THINKING_CANVAS\n{content}\n-------------------------------------------\n")
            canvas_block = "\n\n" + canvas_tmpl.format(content=self.agent.ops._canvas)
