...
from kernel.ui import C, Spinner
...
class OllamaClient:
    ...
    def chat(self, messages: list, system: str = "") -> str:
        ...
        try:
            ...
            # Convert exhausted 429s into a uniform RateLimitExhausted error for callers/spec.
            code = getattr(exc.response, "status_code", None)
            if code == 429:
...
class NvidiaClient:
    ...
    def chat(self, messages: list, system: str | None = None) -> str:
        """chat function."""
        if not self.api_key:
            return "FINAL ANSWER: NVIDIA_API_KEY is not set. Add it to .env or switch the provider to Ollama in cfg.yaml."
        _wait_for_rate_limit(self.cfg, self.provider, self.model)
...
class OpenAICompatibleClient:
    ...
    def __init__(self, cfg: dict, provider: str, env_key: str):
        ...
        if not self.model:
            raise RuntimeError(f"Missing 'model' configuration for {provider} in cfg/providers.yaml")
        ...
        try:
            ...
            if base_url:
                ...
                if provider != "openai":
                    raise RuntimeError(f"Missing 'base_url' configuration for {provider} in cfg/providers.yaml")
    ...
    def chat(self, messages: list, system: str = "") -> str:
        ...
        try:
            ...
            # Convert common rate-limit exceptions into RateLimitExhausted for callers/spec
            try:
                from openai import RateLimitError, APIError
