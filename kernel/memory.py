...
class MemoryManager:
    ...
    def log_task_completion(self, task_goal: str, final_answer: str, ops_used: List[str], 
...
**Tools Used:** {', '.join(ops_used) if ops_used else 'None'}
...
""".strip()
    ...
    def _generate_insights_from_tasks(self, tasks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        tool_usage = {}
        for task in tasks:
            for tool in task.get("ops_used", []):
                tool_usage[tool] = tool_usage.get(tool, 0) + 1
        most_used_ops = sorted(tool_usage.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    def _extract_patterns(self, tasks: List[Dict[str, Any]]) -> List[str]:
        ...
        nouns = ["file", "directory", "web", "api", "data", "code", "script", "cfg", "system", "network"]
        found_nouns = []
        for goal in goals:
            for noun in nouns:
                if noun in goal:
    ...
    def _update_long_term_memory(self, insights: Dict[str, Any], recent_tasks: List[Dict[str, Any]]):
        ...
        if insights:
            ...
            if insights.get('most_used_ops'):
                new_section += f"**Most Used Tools:** {', '.join([tool for tool, count in insights['most_used_ops']])}\n\n"
        ...
        # Comclie with existing content (keep recent content at top)
        if existing_content:
...
def log_task_completion(goal: str, final_answer: str, ops_used: List[str], 
                       success: bool, duration: float, metadata: Optional[Dict] = None, task_id: Optional[str] = None):
    """Convenience function to log task completion."""
    if _memory_manager:
        _memory_manager.log_task_completion(goal, final_answer, ops_used, success, duration, metadata, task_id)
