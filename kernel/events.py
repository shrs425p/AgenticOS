...
class OSEventBus:
    ...
    @tool(name="getosevents", desc="Retrieve the list of recent hardware and system-level events (CPU spikes, battery drops, network transitions).", category="System")
    def getosevents(self) -> str:
    ...
    @tool(name="configureosevents", desc="Set CPU, memory, battery thresholds or polling interval. Args: cpu_threshold (optional), memory_threshold (optional), battery_threshold (optional), poll_interval (optional)", category="System")
    def configureosevents(
        self,
    ...
    @tool(name="stoposeventbus", desc="Shut down the background OS event loop to release resources.", category="System")
    def stoposeventbus(self) -> str:
        """Gracefully halts the background event loop thread."""
        self.stop()
        return "Success: Background OS Event Bus has been shut down."

    @tool(name="startoseventbus", desc="Initialize and start the background OS event loop.", category="System")
    def startoseventbus(self) -> str:
