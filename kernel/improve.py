...
class SelfImprovementDaemon:
    ...
    def _offline_fallback_scan(self) -> List[str]:
        ...
        # Scan ops/addons/ for files with no docstring
    ...
    def _generate_reflections(self, tasks: List[Dict]) -> List[str]:
            ...
            try:
                ...
                if not system:
                    system = (
                        ...
                        "- Environment-specific facts discovered (file paths, cfgs, user preferences)\n\n"
        ...
        for t in tasks:
            for tool in t.get("ops_used", []):
                tool_freq[tool] = tool_freq.get(tool, 0) + 1
        if tool_freq:
            top = sorted(tool_freq.items(), key=lambda x: x[1], reverse=True)[:3]
            tmpl = reflection_prompts.get("tool_usage", "Most relied-on ops: {ops}")
            ops_str = ", ".join(f"{name} ({count}x)" for name, count in top)
            lessons.append(tmpl.format(ops=ops_str))
