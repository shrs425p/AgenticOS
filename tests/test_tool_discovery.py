"""Tests for SemanticToolIndex in core/tool_discovery.py."""

import pytest
from core.tool_discovery import SemanticToolIndex
...
def _make_registry():
    """Return a mock tool registry with 5 tools."""
    return {
        "read_file": {
            "fn": lambda path: "",
...
class TestSemanticToolIndex:
    ...
    def test_build_index(self):
        """Build index from 5 mock tools; verify internal index is populated."""
    ...
    def test_rebuild_updates_index(self):
        """Build once, rebuild with a different registry; old tools should be gone."""
        index = SemanticToolIndex()
        index.build(_make_registry())

        # Confirm original tools present
    ...
    def test_search_scores_are_positive(self):
        """All returned scores should be strictly positive."""
        index = SemanticToolIndex()
        index.build(_make_registry())
