...
def test_fast_disk_audit_functional():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        large_file = os.path.join(users_dir, "large_file.bin")
        ...
        # Test large mode
        res_large = fast_disk_audit(path=tmpdir, top_n=5, min_mb=100, mode="large")
        assert "TOP 5 LARGEST FILES" in res_large
        assert "large_file.bin" in res_large
        
        # Test duplicates mode
        res_dupes = fast_disk_audit(path=tmpdir, mode="duplicates")
        assert "DUPLICATE FILENAMES" in res_dupes
        assert "duplicate.txt" in res_dupes
        assert "2" in res_dupes
        
        # Test old mode
        res_old = fast_disk_audit(path=tmpdir, mode="old")
