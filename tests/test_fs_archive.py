...
def test_zip_and_unzip(tmp_path):
    ...
    # 1. Test zip_files
    res = tool.zip_files(str(zip_out), str(f1), str(sub_dir), "nonexistent.txt")
    assert "Created zip:" in res
    assert zip_out.exists()
    
    # 2. Test unzip_file
    dest_dir = tmp_path / "dest"
    res2 = tool.unzip_file(str(zip_out), str(dest_dir))
...
def test_archive_exceptions(tmp_path):
    ...
    res = tool.zip_files(str(tmp_path), "nonexistent.txt")
    assert "Zip error" in res
    
    # Test unzip exception
    res2 = tool.unzip_file("nonexistent.zip", str(tmp_path / "dest"))
