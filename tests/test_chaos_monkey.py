...
from core.audit_logger import infer_success
...
class ChaosMonkey:
    ...
    def __enter__(self):
        ...
        # 3. LLM API timeout
        if self.llm_timeout:
            from core import model_clients
            ...
            for attr in dir(model_clients):
                val = getattr(model_clients, attr)
...
def test_agent_tool_retry_simulation():
    ...
    # Mock tools.call to fail first with SQLite lock and then succeed
    call_sequence = [
        "Error: database is locked",
        "Success: wrote record"
    ]
    agent.tools.call.side_effect = call_sequence
    ...
    started = time.time()
    obs = agent.tools.call(tool_name, args)
    ended = time.time()
    ...
    assert agent.tools.call.call_count == 2
    assert ok is True
