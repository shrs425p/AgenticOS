"""Tests for StreamingActionParser and ParallelScheduler in core/dispatcher.py."""
...
class TestStreamingActionParser:
    ...
    def test_streaming_parser_simple_json(self):
        ...
        text = 'ACTION: {"tool": "run_command", "args": {"cmd": "ls"}}'
...
class TestParallelScheduler:
    ...
    def test_parallel_scheduler_with_deps(self):
        ...
        actions = [
            ...
            {"tool": "tool_b", "args": {}, "depends_on": [0]},  # index 1 — depends on 0
        ]
        ...
        assert results[0] == "done_tool_a"
        assert results[1] == "done_tool_b"
        # tool_a must have completed before tool_b started (Kahn's wave ordering)
        assert completion_order.index("tool_a") < completion_order.index("tool_b")
