"""Tests for the url_safety_check plugin."""
from unittest.mock import MagicMock, patch
import datetime
from tools.plugins.url_safety_check import (
...
@patch("tools.plugins.url_safety_check._get_raw_whois")
@patch("ssl.create_default_context")
def test_url_safety_check_secure_and_old(mock_ssl_context, mock_whois):
    ...
    result = url_safety_check("https://secure-domain.com")
    ...
    assert "Final Risk Score**: `0 / 10`" in result
    assert "SECURE & SAFE" in result


@patch("tools.plugins.url_safety_check._get_raw_whois")
def test_url_safety_check_insecure_http_new_domain(mock_whois):
    ...
    result = url_safety_check("http://insecure-fresh-domain-highly-complex-redirect-phishing-url.com")
    ...
    assert "Final Risk Score" in result
    assert int(result.split("Final Risk Score**: `")[1].split(" / 10`")[0]) >= 5
