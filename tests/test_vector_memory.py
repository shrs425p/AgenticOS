...
def test_exponential_time_decay(test_namespace):
    ...
    fresh_score = results[0]["similarity"]
    aged_30_score = results[1]["similarity"]
    aged_60_score = results[2]["similarity"]
    
    # Verify half-life of 30 days
    assert pytest.approx(aged_30_score, abs=0.05) == fresh_score * 0.5
    assert pytest.approx(aged_60_score, abs=0.05) == fresh_score * 0.25
...
@pytest.mark.skipif(not HAS_NUMPY, reason="numpy is not installed")
@patch.object(VectorDB, 'get_embedding', new=mock_get_embedding)
def test_cluster_memories_tool(test_namespace):
    from tools.plugins.vector_memory import cluster_memories
    
    vector_memory_store(test_namespace, "I like eating an apple", metadata="fruit")
    vector_memory_store(test_namespace, "Peeling a banana", metadata="fruit")
    vector_memory_store(test_namespace, "Sweet red apples", metadata="fruit")
    
    vector_memory_store(test_namespace, "Driving a fast car", metadata="vehicle")
    vector_memory_store(test_namespace, "A large truck", metadata="vehicle")
    vector_memory_store(test_namespace, "Red sports cars", metadata="vehicle")
    
    clusters_str = cluster_memories(test_namespace, k=2)
...
def test_cross_instance_sharing():
    ...
    try:
        vector_memory_store(shared_namespace, "shared apple memory", metadata="shared_fruit", shared=True)
        assert os.path.exists(db_path)
