...
def test_sentinel_load_config_fail(tmp_path):
    ...
    with open(s.log_path, "r", encoding="utf-8") as f:
        content = f.read()
        assert "Failed to load config" in content
...
def test_sentinel_pre_check_blocked(tmp_path):
    ...
    s = Sentinel(str(tmp_path), config_path=str(cfg_path), alert_callback=callback)
