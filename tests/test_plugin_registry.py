...
from core.plugin_registry import PluginRegistryClient
...
@tool(name="test_temp_tool", desc="A temporary test tool")
def test_temp_tool(val: str) -> str:
...
"""
    ...
    with patch("requests.get", side_effect=mock_get), \
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            assert "test_temp_tool" in f.read()

        # Verify dynamic load registration was triggered
        assert mock_registry._register_module_tools.called
        
        # Verify the module is in sys.modules and callable
        module = sys.modules.get("tools.plugins.test_temp_plugin")
        assert module is not None
        assert hasattr(module, "test_temp_tool")
        assert module.test_temp_tool("test") == "Processed test"
