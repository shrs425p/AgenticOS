...
AgenticOS natively supports the following providers through the `kernel/models.py` interface:
...
In production, API rate limits are a common cause of agent failure. AgenticOS centralizes exponential backoff and retry behavior in `kernel/retry.py` as `retry_call()`. Provider clients (for example, Nvidia and OpenAI-compatible adapters) call `retry_call()` to handle transient 429/RateLimit errors and apply jittered exponential backoff. If retries are exhausted the client will raise `RateLimitExhausted`, allowing the orchestrator to decide on fallbacks or recovery strategies.
...
2.  **Wait**: The system pauses for a calculated duration (cfgurable via `base_retry_delay`).
3.  **Backoff**: If it fails again, the wait time increases exponentially with randomized jitter.
4.  **Max Retries**: The system attempts up to `max_retries` times (cfgurable) before reporting a failure or raising `RateLimitExhausted`.
...
-   **Fix**: Increase `num_ctx` in `cfg.yaml` or use the `read_file` line-range parameters to read smaller chunks.
