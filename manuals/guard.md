...
If an agent attempts to run `findlargefiles` or `grep_dir` on the root `<SYSTEM_DRIVE>\` drive using standard Python libraries, the system will **block the execution**.
-   **Rationale**: Python-based file walkers are too slow for millions of files and cause 100% SSD usage.
-   **Enforcement**: The agent is told to use the optimized `fastdiskaudit` tool instead.
...
The `runcommand`, `runpowershell`, and `runscript` ops are the most powerful and dangerous in the registry. To prevent command injection and obfuscation attacks, AgenticOS enforces a zero-trust command validation layer (`SafetyMixin` in [safety.py](../ops/terminal/safety.py)).
...
For detailed validation and script hardening mechanisms, refer to the [Command Validation Guide](commands.md).
...
### How it works:
- **Regex Scanning**: Every tool output, observation, and thought block is scanned against a list of high-sensitivity regex patterns (defined in `cfg/policy.yaml`).
...
To achieve maximum protection when interacting with foreign codebases or untrusted remote endpoints, AgenticOS integrates two kernel security and sandbox audit systems:

### ◆ 1. Cryptographic SSL & WHOIS Threat Skernelr (`urlsafetycheck`)
...
### ◆ 2. Dynamic Sandbox Auditing (`ossandboxauditor`)
...
## Security Configuration (`cfg/policy.yaml`)

Security settings are now centralized in the `cfg/` directory. The primary file is `policy.yaml`:
...
1.  **Always use `_resolve()`**: When writing new ops, always resolve paths through the `FileManager` to ensure they are checked against the guardrails.
...
4.  **Use the manuals catalog**: Keep security updates synced in [CATALOG.md](CATALOG.md) and [SECURITY.md](../SECURITY.md) so operators can find current guidance quickly.
