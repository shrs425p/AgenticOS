...
-   **Fix**: Use the `findlargefiles` tool. We implemented **Performance Guardrails** that should now block these slow scans and suggest the faster native method.
...
-   **Fix**:
    ...
    -   Check the `PathGuard` settings in `cfg.yaml` to ensure the path isn't in the **Red Zone**.
...
-   **Fix**: Enable `self_healing: true` in `cfg.yaml`. The system will detect the invalid JSON and ask the model to re-format it.

---

## Diagnostic Tools

AgenticOS includes several built-in ops to help you diagnose its health:

| Tool | Purpose |
| :--- | :--- |
| `systemhealth` | Full report on CPU, RAM, Disk, and Agent process stats. |
| `validate_cfg` | Verifies the runtime configuration is correctly formed. |
| `system_info` | Provides overall system diagnostics. |
| `selfprocessinfo` | Shows the agent's own PID, memory, and handles. |
...
-   **Fix**: Update the `_MACOS_KEYCODE` dictionary inside `ops/terminal/keyboard.py` to map your custom key name, or fall back to passing the key as a standard text string/keystroke.

### 5. "Missing screen capture ops error on Linux."
-   **Cause**: Linux environments do not ship with screenshot CLI ops by default.
-   **Fix**: Install `scrot` or `gnome-screenshot` using your native package manager (e.g. `sudo apt install scrot` or calling `installsystempackage('scrot')`).
...
3.  **Reset Config**: Delete `cfg.yaml` and restart; the system will generate a fresh copy with safe defaults.
