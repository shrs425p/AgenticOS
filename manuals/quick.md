...
The agent will design an execution plan, run the necessary system telemetry ops, analyze the output, and display the formatted results in your terminal.
...
Heuristics:
*   Executes `websearch` to query technical search engines.
...
Heuristics:
*   Uses optimized filesystem ops to scan the active workspace.
...
*   **Custom Configurations**: See the [Runtime Configuration Guide](runtime.md) to tweak YAML profiles.
*   **Security Policy**: Review the [Security and Guardrails Manual](guard.md) to understand PathGuard zones.
*   **API reference**: Look at the [API Reference Catalog](api.md) to explore the 350+ built-in system ops.
