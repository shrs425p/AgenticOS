...
The Self-Improvement engine is driven by the `SelfImprovementDaemon` ([self_improvement.py](../kernel/improve.py)). Rather than executing tasks in a historical vacuum, the agent conducts a "Dream Cycle" to consolidate experience:
...
If a model client is connected, the daemon formats the target task goals, ops used, durations, and result snippets into a structured history report. It prompts the LLM to extract exactly 3 to 5 highly specific lessons:
...
*   **AST Plugin Verification**: Uses Python's `ast` module to parse files in `ops/addons/` and check for functions missing Google-style docstrings (which could cause execution loop model mapping failures).
