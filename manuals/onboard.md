...
Welcome to the AgenticOS developer onboarding guide. This document contains standard practices for contributing code, developing ops/addons, running formatting ops, and submitting pull requests.
...
The project uses Python scripts and standard ops for testing and formatting.
...
- **Naming Convention**:
  - `feat/feature-name` for new features/ops.
  - `fix/bug-fix-name` for bug fixes.
  - `manuals/manuals-update` for documentation changes.
...
2. **Implement Changes**: Write clean, modular code. If writing ops, decorate them with `@tool` and register them type-safely.
3. **Write Unit Tests**: Add test assertions in the `spec/` directory matching the naming convention `test_*.py`.
4. **Run Verification**: Ensure all unit spec pass cleanly:
   ```bash
   venv\Scripts\pytest
   ```
